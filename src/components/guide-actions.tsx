...
import { deleteGuide } from "@/app/guides/actions";
...
export function GuideActions({
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        window.alert("Link panduan dibagikan");
      } else {
        await navigator.clipboard.writeText(url);
        window.alert("Link panduan disalin");
      }
    } catch {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        disabled={pending}
        onClick={() => startTransition(async () => deleteGuide(id))}
